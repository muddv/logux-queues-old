...
export const COMMENTS: Map<string, CommentType> = new Map()
...
export default (server: Server) => {
		...
		filter(ctx, action) {
			return (otherCtx, otherAction) => {
				if (createdComment.match(otherAction)) {
					...
					deletedComment.match(otherAction) ||
	...
	server.channel<{ id: string }>('comments/:id', {
		access(ctx) {
			return true
		}
	})
	server.type(createComment, {
		...
		async process(ctx, action) {
			...
			await server.process(
				createdComment({ id: action.id, fields: action.fields })
			)
		}
	})
  ...
  server.type(changeComment, {  
    ...
    async process(ctx, action) {                                                             
      ...
      await server.process(                    
        changedComment({ id: action.id, fields: action.fields })  
      )                                        
    }                                          
  }) 
